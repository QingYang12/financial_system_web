<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <span><b>消费金额:</b></span><el-input v-model="listQuery.money" placeholder="消费金额" style="width: 140px;margin-right:50px;" class="filter-item"  clearable />
      <span><b>发生时间:</b></span>
      <el-date-picker
    ...
    >
      ...
      <el-table-column label="消费类别" prop="type" sortable="custom" align="center"  :class-name="getSortClass('id')">
        <template slot-scope="{row}">
          <span>{{ row.type }}</span>
        </template>
      </el-table-column>
      <el-table-column label="消费金额" prop="money" sortable="custom" align="center"  :class-name="getSortClass('id')">
        <template slot-scope="{row}">
          <span>{{ row.money }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="80px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="消费类别" prop="type">
          <el-select v-model="temp.type" placeholder="请选择" style="width: 150px;margin-right:50px;" >
            <el-option
              v-for="item in options"
              :key="item.value"
              :label="item.label"
...
<script>
import { fetchconsumptionList, fetchUser,deleteconsumption, createconsumption, updateconsumption } from '@/api/financial'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchconsumptionList(params).then(response => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createconsumption(this.temp).then(() => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          updateconsumption(this.temp).then(() => {
            this.dialogFormVisible = false
            this.$notify({
              title: 'Success',
